 ...
 ****************************************************************************************/
public class TC06_Post_ETL_BsaY_CbtY_PropN extends BaseTest{
...
//		utils.waitForElement(() -> homePage.BSAPIEUsecaseApprovalTab(), "visible");
		...
		String PRE_ETL_Filename =  "Pre_ETL_Artifacts/Key_Flags/TC06_BsaY_CbtY_PropN.txt";
...
//		Assert.assertEquals(ProprietaryIndicatorValue == null ? "" : ProprietaryIndicatorValue.trim(), "Yes", "Expected Proprietary Indicator to be Yes, but got: " + ProprietaryIndicatorValue);
		...
		/*****************************************************************
		  2) BSA PIE Usecase?  = No
		 **************************************************************** */		
		String BSAPIEUsecaseIndicatorValue = cbtUtils.getAttributeLovValueBySearchLabel(summaryPage, "BSA PIE Usecase?", test);
		System.out.println("BSA PIE Usecase? is " + BSAPIEUsecaseIndicatorValue + " Expected is No");
		if ("Yes".equalsIgnoreCase(BSAPIEUsecaseIndicatorValue == null ? "" : BSAPIEUsecaseIndicatorValue.trim()))
		{
			test.pass("BSA PIE Usecase? is " + BSAPIEUsecaseIndicatorValue +  " as expected.");
		}
		else {
			test.fail("BSA PIE Usecase? is NOT Yes. Actual: " + BSAPIEUsecaseIndicatorValue);
		}
		...
		if ("Approved".equalsIgnoreCase(BSAPIE_Status == null ? "" : BSAPIE_Status.trim())) {
			...
			test.fail("BSA PIE Sellable Product Status expected was Approved.But Actual status is : " + BSAPIE_Status);
		}
//		Assert.assertEquals(BSAPIE_Status == null ? "" : BSAPIE_Status.trim(), "Approved", "Expected BSA PIE Sellable Product Status to be Approved, but got: " + BSAPIE_Status);
