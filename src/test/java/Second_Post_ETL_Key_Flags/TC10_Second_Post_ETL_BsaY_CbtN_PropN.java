 ...
 ****************************************************************************************/
public class TC10_Second_Post_ETL_BsaY_CbtN_PropN extends BaseTest{
...
//		Assert.assertEquals(BSAPIEUsecase_Value == null ? "" : BSAPIEUsecase_Value.trim(), "No", "Expected Munitions Indicator to be No, but got: " + BSAPIEUsecase_Value);
		/*****************************************************************
		  2) BSA PIE Sellable Product Status = ProprietarySyndicated
		 **************************************************************** */
		...
		if ("ProprietarySyndicated".equalsIgnoreCase(bsaPieStatus == null ? "" : bsaPieStatus.trim())) {
			test.pass("BSA PIE Sellable Product Status is ProprietarySyndicated as expected.");
		}
		else
		{
			test.fail("BSA PIE Sellable Product Status is NOT ProprietarySyndicated. Actual: " + bsaPieStatus);
		}
//		Assert.assertEquals(bsaPieStatus == null ? "" : bsaPieStatus.trim(), "ProprietarySyndicated", "Expected BSA PIE Sellable Product Status to be ProprietarySyndicated, but got: " + bsaPieStatus);
		Thread.sleep(2000);
